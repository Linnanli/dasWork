...
function runtimePresentationCommandSource(
  ...
  return [
    ...
    "const renderEnv = { ...pythonEnv, FONTCONFIG_FILE: fontConfig, FONTCONFIG_PATH: dirname(dependencies.font), PPTX_RUNTIME_SOFFICE: dependencies.soffice, PPTX_RUNTIME_PDFTOPPM: dependencies.pdftoppm, ...(process.platform === 'win32' ? { APPDATA: join(libreOfficeProfile, 'AppData', 'Roaming'), LOCALAPPDATA: join(libreOfficeProfile, 'AppData', 'Local'), PPTX_RUNTIME_SOFFICE_USER_INSTALLATION: `-env:UserInstallation=${pathToFileURL(libreOfficeProfile).href}`, USERPROFILE: libreOfficeProfile } : {}), PATH: [dirname(dependencies.soffice), dirname(dependencies.pdftoppm), ...runtimeSystemPaths].join(delimiter) }",
