...
type HostRequestResult = Promise<unknown> | object | string | number | boolean | null | undefined

export type HostRequestHandler = (params: unknown, request: JsonRpcRequest) => HostRequestResult
...
export type CodexAppServerConnectionDiagnostics = {
  ...
  hostRequestHandlerCount: number
}
...
const HOST_REQUEST_THREAD_NOT_FOUND_ERROR = {
  code: -32002,
  message: 'Host capability request references an inactive thread.'
}

function methodNotFoundError(method: string): { code: number; message: string } {
  return {
    code: -32601,
...
export class CodexAppServerConnectionBroker {
  ...
  private readonly cancelledWireIds = new Set<JsonRpcId>()
  ...
  private lateResponseCount = 0
  private readonly hostRequestHandlers = new Map<string, HostRequestHandler>()
  ...
  cancelRequest(channel: BrokerLogicalChannel, localId: JsonRpcId): void {
    for (const [wireId, pending] of this.pendingRequests) {
      if (pending.channel === channel && pending.localId === localId) {
        this.markRequestCancelled(wireId)
        return
      }
    }
  ...
  registerHostRequestHandler(method: string, handler: HostRequestHandler): () => void {
    if (this.hostRequestHandlers.has(method)) {
      throw new Error(`Host request handler already registered for ${method}.`)
    }
  ...
  getDiagnostics(): CodexAppServerConnectionDiagnostics {
    return {
      ...
      hostRequestHandlerCount: this.hostRequestHandlers.size
    }
  }
  ...
  private async sendInitialize(
    ...
    if (this.initializeState === 'initialized') {
      queueMicrotask(() =>
        channel.receiveMessage({ id: message.id, result: this.initializeResult })
      )
      return
    }
  ...
  private handlePhysicalMessage(transport: CodexTransport, message: JsonRpcMessage): void {
    ...
    if (isResponse(message)) {
      ...
      if (!pending) {
        if (this.cancelledWireIds.delete(message.id)) {
  ...
  private routeServerRequest(request: JsonRpcRequest): void {
    ...
    const hostHandler = this.hostRequestHandlers.get(request.method)
    if (hostHandler) {
      if (target.threadId && !this.threadOwners.has(target.threadId)) {
        this.respondToServerRequest(request.id, { error: HOST_REQUEST_THREAD_NOT_FOUND_ERROR })
        return
      }
      void this.handleHostRequest(request, hostHandler)
      return
    }
    const owner =
      target.turnId && target.threadId
    ...
    void this.transport
      ?.sendMessage({
        id: request.id,
        error:
          target.threadId || target.turnId ? NO_OWNER_ERROR : methodNotFoundError(request.method)
      })
      .catch(() => undefined)
  }
  ...
  private async handleHostRequest(
    ...
    try {
      ...
      this.respondToServerRequest(request.id, {
        error: {
          code: -32000,
          message: error instanceof Error ? error.message : 'Host request handler failed.'
        }
      })
    }
  }
  ...
  private respondToServerRequest(
    ...
    void this.transport?.sendMessage({ id, ...response }).catch(() => undefined)
  }
  ...
  private markRequestCancelled(wireId: JsonRpcId): void {
    if (!this.pendingRequests.delete(wireId)) {
      return
    }
    this.cancelledWireIds.add(wireId)
    this.cancelledRequestCount += 1
  }
...
function isInitializeRequest(message: JsonRpcMessage): message is InitializeRequest {
