...
import type { BrokerLogicalChannel, CodexAppServerConnectionBroker } from './connection-broker'
import type { CodexTransport, CodexTransportEventMap, JsonRpcMessage } from './transport'
...
export class PersistentTransport implements CodexTransport, BrokerLogicalChannel {
  ...
  private brokerAbortListener: (() => void) | undefined
  ...
  constructor(settings: PersistentTransportSettings) {
    ...
    if (this.pool && this.broker) {
      throw new Error(
        'PersistentTransport cannot use a worker pool and connection broker together.'
      )
    }
  ...
  async connect(): Promise<void> {
    if (this.broker) {
      ...
      if (this.signal?.aborted) {
        throw brokerAbortError()
      }
      this.onLeaseRequested?.()
      this.leaseActive = true
      this.addBrokerAbortListener()
      try {
        ...
        this.removeBrokerAbortListener()
        this.releaseLease()
        throw error
      }
      return
  ...
  on<K extends keyof CodexTransportEventMap>(
    ...
    if (this.broker) {
      ...
      const closeListener = listener as (code: number | null, signal: NodeJS.Signals | null) => void
  ...
  private addBrokerAbortListener(): void {
    if (!this.signal || this.brokerAbortListener) {
      return
    }
    this.brokerAbortListener = () => this.abortBrokerChannel()
    this.signal.addEventListener('abort', this.brokerAbortListener, { once: true })
  }

  private removeBrokerAbortListener(): void {
    if (!this.signal || !this.brokerAbortListener) {
      return
    }
    this.signal.removeEventListener('abort', this.brokerAbortListener)
    this.brokerAbortListener = undefined
  }

  private abortBrokerChannel(): void {
    if (!this.broker) {
      return
    }
    const error = brokerAbortError()
    if (this.brokerConnected) {
      this.brokerConnected = false
    }
    this.broker.detach(this)
...
function brokerAbortError(): CodexRequestCancelledError {
  return new CodexRequestCancelledError('Codex app-server logical transport aborted.')
}
