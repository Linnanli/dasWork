...
import { CodexRequestCancelledError } from './errors'
...
type McpServerStatusRequestEntry = {
  promise: Promise<CodexMcpServerStatusSummary[]>
...
export interface CodexContextCatalogClientSettings extends CodexAppServerClientSettings {
   ...
   */
  acquireClient?: (context: Readonly<TransportContext>) => Promise<CodexContextCatalogClientLease>
  /**
...
export class CodexContextCatalogClient {
  ...
  private readonly mcpServerStatusRequests = new Map<string, McpServerStatusRequestEntry>()
  ...
  async listSkills(
    ...
    context: Readonly<TransportContext> = {}
  ): Promise<CodexCatalogSkill[]> {
  ...
  async listPluginCatalog(
    params: CodexPluginCatalogListParams = {},
    context: Readonly<TransportContext> = {}
  ): Promise<PluginListResponse> {
    return this.withClient(
      (client) =>
        client.request<PluginListResponse>(
          'plugin/list',
          stripUndefined({
  ...
  async readPluginDetailsForManagement(
    params: CodexPluginCatalogDetailsParams = {}
  ): Promise<PluginDetail[]> {
    return this.withClient(async (client) => {
      const catalog = await client.request<PluginListResponse>(
        'plugin/list',
        stripUndefined({
          cwds: params.cwd ? [params.cwd] : undefined,
          forceRefetch: params.forceRefetch,
      ...
      return this.readPluginDetailsFromCatalogWithClient(
        client,
        catalog,
        params.onlyInstalled === true
      )
    })
  }
   ...
   */
  async readPluginDetailForManagement(
    params: CodexPluginDetailReadRequest,
    context: Readonly<TransportContext> = {}
  ): Promise<PluginDetail> {
    assertPluginDetailReadRequest(params)
    return this.withClient(async (client) => {
      const response = await client.request<PluginReadResponse>(
        'plugin/read',
        stripUndefined({
  ...
  async listInstalledPluginsForManagement(
    params: { cwd?: string } = {},
    context: Readonly<TransportContext> = {}
  ): Promise<PluginInstalledResponse> {
    return this.withClient(
      (client) =>
        client.request<PluginInstalledResponse>('plugin/installed', {
  ...
  async readInstalledPluginDetails(params: { cwd?: string } = {}): Promise<PluginDetail[]> {
    return this.withClient(async (client) => {
      ...
      for (const plugin of installedPlugins) {
        const detail = await client.request<PluginReadResponse>(
          'plugin/read',
          stripUndefined(plugin)
        )
  ...
  async readSkillFileContents(
    params: CodexSkillFileReadParams,
    context: Readonly<TransportContext> = {}
  ): Promise<string> {
    ...
    return this.withClient(async (client) => {
      const response = await client.request<FsReadFileResponse>('fs/readFile', {
        path
      } satisfies FsReadFileParams)
      return decodeBoundedSkillContents(response.dataBase64, maxBytes)
    }, context)
  }

  async readRemotePluginSkillContents(
    params: CodexRemotePluginSkillReadParams,
    context: Readonly<TransportContext> = {}
  ): Promise<string | null> {
    ...
    const maxBytes = skillContentsMaxBytes(params.maxBytes)
    return this.withClient(async (client) => {
      const response = await client.request<PluginSkillReadResponse>('plugin/skill/read', {
        remoteMarketplaceName,
        remotePluginId,
        skillName
      } satisfies PluginSkillReadParams)
      return response.contents === null ? null : boundedSkillContents(response.contents, maxBytes)
    }, context)
  }

  async listAppsForManagement(
    ...
    return this.withClient(async (client) => {
      try {
        ...
        if (
          !isUnsupportedMethod(error, 'app/installed') &&
          !isUnsupportedMethod(error, 'app/read')
        ) {
          throw error
        }
      }
  ...
  async readAppsForManagement(
    params: CodexAppsReadParams,
    context: Readonly<TransportContext> = {}
  ): Promise<CodexAppsManagementReadResult> {
  ...
  async listAppsManagementPage(
    ...
    return this.withClient(
      (client) => this.requestAppsManagementPage(client, params, params.cursor),
      context
    )
  }

  async readConfigForManagement(
    params: { cwd?: string } = {},
    context: Readonly<TransportContext> = {}
  ): Promise<ConfigReadResponse> {
    return this.withClient((client) => this.readConfig(client, params.cwd), context)
  }

  async readMcpManagementSnapshot(
    params: { cwd?: string } = {},
    context: Readonly<TransportContext> = {}
  ): Promise<CodexMcpManagementSnapshot> {
    const [config, servers] = await Promise.all([
      this.readConfigForManagement(params, context),
      this.listMcpServerStatus({}, context).catch((error) => {
        if (isCodexRequestCancellation(error)) {
          throw error
        }
        return []
      })
  ...
  async installPlugin(params: CodexPluginInstallRequest): Promise<PluginInstallResponse> {
    ...
    return this.withClient((client) =>
      client.request<PluginInstallResponse>('plugin/install', request)
    )
  }

  async uninstallPlugin(params: PluginUninstallParams): Promise<PluginUninstallResponse> {
    return this.withClient((client) =>
      client.request<PluginUninstallResponse>('plugin/uninstall', params)
    )
  }
  ...
  async setPluginEnabled(params: {
    ...
    return this.writeEnabledConfigValue(
      params.cwd,
      ['plugins', params.pluginId, 'enabled'],
      params.enabled
    )
  }

  async setAppEnabled(params: {
    cwd?: string
    appId: string
    enabled: boolean
  }): Promise<CodexConfigWriteActionResult> {
    return this.writeEnabledConfigValue(
      params.cwd,
      ['apps', params.appId, 'enabled'],
      params.enabled
    )
  }

  async setMcpServerEnabled(params: {
    cwd?: string
    serverName: string
    enabled: boolean
  ...
  async setSkillEnabled(params: CodexSkillEnabledRequest): Promise<SkillsConfigWriteResponse> {
    ...
    return this.withClient((client) =>
      client.request<SkillsConfigWriteResponse>('skills/config/write', request)
    )
  }
  ...
  async addMarketplace(params: MarketplaceAddParams): Promise<MarketplaceAddResponse> {
    ...
    return this.withClient((client) =>
      client.request<MarketplaceAddResponse>(
        'marketplace/add',
        stripUndefined({
  ...
  async listMcpServerStatus(
    ...
    if (context.signal?.aborted) {
      throw new CodexRequestCancelledError()
    }
    ...
    if (existingRequest) {
      return this.waitForMcpServerStatusRequest(requestKey, existingRequest, context.signal)
    }

    const ownerController = new AbortController()
    const entry: McpServerStatusRequestEntry = {
      promise: Promise.resolve([]),
      ownerController,
      consumers: new Set()
    }
    entry.promise = this.withClient(
      async (client) => {
      ...
      { ...context, signal: ownerController.signal }
    )
    this.mcpServerStatusRequests.set(requestKey, entry)
    const clearRequest = (): void => {
      if (this.mcpServerStatusRequests.get(requestKey) === entry) {
        this.mcpServerStatusRequests.delete(requestKey)
      }
    }
    void entry.promise.then(clearRequest, clearRequest)
    return this.waitForMcpServerStatusRequest(requestKey, entry, context.signal)
  }

  private waitForMcpServerStatusRequest(
    requestKey: string,
    entry: McpServerStatusRequestEntry,
    signal: AbortSignal | undefined
  ): Promise<CodexMcpServerStatusSummary[]> {
    if (signal?.aborted) {
      return Promise.reject(new CodexRequestCancelledError())
    }

    const consumer = Symbol('mcp-server-status-consumer')
    entry.consumers.add(consumer)
    return new Promise<CodexMcpServerStatusSummary[]>((resolve, reject) => {
      let settled = false
      const settle = (action: () => void): void => {
        ...
        if (entry.consumers.size === 0 && this.mcpServerStatusRequests.get(requestKey) === entry) {
          entry.ownerController.abort()
          this.mcpServerStatusRequests.delete(requestKey)
        }
        action()
      }
      const abort = (): void => settle(() => reject(new CodexRequestCancelledError()))
      signal?.addEventListener('abort', abort, { once: true })
      entry.promise.then(
        (servers) => settle(() => resolve(servers)),
        (error) => settle(() => reject(mcpServerStatusRequestError(error)))
      )
    })
  }
  ...
  async searchThreads(params: { query: string; limit?: number }): Promise<CodexTaskSearchResult[]> {
    return this.withClient(async (client) => {
      ...
      return response.data.map(({ thread, snippet }) =>
        stripUndefined({
          threadId: thread.id,
  ...
  private async requestAppsPage(
    ...
    return stripUndefined({
      data: response.data.filter((app) => app.isEnabled && app.isAccessible).map(normalizeApp),
  ...
  private async requestMcpServerStatusPage(
    ...
    const response = await client.request<ListMcpServerStatusResponse>(
  ...
  private async writeEnabledConfigValue(
    ...
    return this.withClient((client) =>
      this.writeConfigValueWithClient(client, cwd, keySegments, enabled, 'upsert')
    )
  }
  ...
  private async readConfig(
    ...
    return client.request<ConfigReadResponse>(
      'config/read',
      stripUndefined({
        includeLayers: true,
        cwd
  ...
  private async withClient<T>(
    ...
    if (this.settings.acquireClient) {
      const lease = await this.settings.acquireClient(context)
      try {
    ...
    if (this.settings.connectionLifecycle === 'per-operation') {
      return this.withLeasedClient(callback, context)
    }
  ...
  async shutdown(): Promise<void> {
    await Promise.allSettled([...this.fuzzyFileSearchSessionStops].map((stop) => stop()))
    const connected = this.clientPromise
    this.clientPromise = undefined
    if (!connected) {
      return
...
function isCodexRequestCancellation(error: unknown): boolean {
  return (
    error instanceof CodexRequestCancelledError ||
    (typeof error === 'object' &&
      error !== null &&
      'code' in error &&
      (error as { code?: unknown }).code === 'app_server_request_cancelled')
  )
}

function mcpServerStatusRequestError(error: unknown): Error {
  return error instanceof Error
    ? error
    : new Error('MCP server status request failed.', { cause: error })
}
...
function isUnsupportedMethod(error: unknown, method: string): boolean {
  ...
  return (
    ...
    (message.includes('unknown variant') && message.includes(method.toLowerCase()))
  )
}
...
function configKeyPath(segments: string[]): string {
  ...
  return segments
    .map((segment, index) =>
      index === 0 || isStaticLeafSegment(segment) ? segment : quoteConfigKeySegment(segment)
    )
    .join('.')
}
