...
export class CodexProcessSessionClient {
  ...
  async shutdown(): Promise<void> {
    ...
    await Promise.all(
      [...this.sessions.values()].map((session) => session.kill().catch(() => undefined))
    )
    this.sessions.clear()
  ...
  private async initializeConnection(): Promise<void> {
    ...
    this.client = client
    this.removeOutputListener = client.onNotification('process/outputDelta', (params) =>
      this.routeOutput(params)
    )
    this.removeExitListener = client.onNotification('process/exited', (params) =>
      this.routeExit(params)
    )
    ...
    try {
      ...
      await client.request<CodexInitializeResult>(
        'initialize',
...
class ProcessSession implements CodexProcessSession {
  ...
  resize(cols: number, rows: number): Promise<void> {
    ...
    return this.client
      .request('process/resizePty', params, this.requestTimeoutMs)
      .then(() => undefined)
  }
  ...
  kill(): Promise<void> {
    ...
    return this.client
      .request('process/kill', { processHandle: this.processHandle }, this.requestTimeoutMs)
      .then(() => undefined)
  }
