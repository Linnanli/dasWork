...
import type { Thread, ThreadItem, Turn } from '@dascowork/codex-app-server-client'
import {
...
function turnDiffPartForTurn(turn: CodexTurnForUi, cwd?: string): DynamicToolUiPart | null {
  ...
  const diff = typeof turn.diff === 'string' ? turn.diff : unifiedDiffForFileChangeBatches(batches)
  if (!diff) {
    return null
  }
...
function agentMessagePart(
  ...
  return {
    ...
    text,
    state: 'done',
    ...(Object.keys(metadata).length > 0 ? { providerMetadata: asProviderMetadata(metadata) } : {})
  }
}
...
function providerMetadataForImageGeneration(
  ...
  return Object.keys(metadata).length > 0 ? asProviderMetadata(metadata) : undefined
}

function asProviderMetadata(metadata: Record<string, unknown>): SharedV3ProviderMetadata {
