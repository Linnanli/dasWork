...
- `desktop-app/src/main/`：Electron main process；负责窗口、IPC payload 校验、外链限制、runtime 生命周期、模型目录、审批转发，以及原生 `CodexRunDriver` 的 thread/turn 编排。
- `desktop-app/vendors/codex-app-server-client/`：项目自有、AI-free 的 app-server transport、协议类型和中性事件 normalizer；它不是 Codex CLI 附带的 JavaScript 库。
- `desktop-app/vendors/ai-sdk-provider-codex-asp/`：仅保留给仓库外 AI SDK consumer 的兼容包；不得重新接入桌面生产聊天路径。
...
- 启动与模型列表：Renderer 调 `desktopCodex.listModels()` -> Preload `codex:list-models` -> Main `CodexChatRuntimeService.listModels()` -> `NativeCodexRunDriver`/`HostCodexConnection` -> app-server 或 admin backend -> 归一化为 `CodexModelList` 返回 UI。
- 聊天流：assistant-ui -> `ElectronIpcChatTransport` -> Preload `desktopCodexChat.startChatStream()` -> `codex-chat:start` + `MessagePort` -> Main Zod 校验 -> `CodexChatRuntimeService` -> `NativeCodexRunDriver` -> AI-free client -> stdio JSON-RPC -> `codex-app-server` -> custom model provider。中性事件经 `CodexUiMessageAdapter` 转回既有 `UIMessageChunk`。
- 审批流：app-server 发出 command/file/tool/mcp 等 server request -> `NativeCodexRunDriver` 的穷举路由 -> `CodexApprovalBroker` -> Main `webContents.send('codex:approval-request')` -> Renderer 审批面板 -> `codex:respond-approval` -> broker resolve -> app-server。
...
- 原生运行时的协议状态、`initialize`、version probe 与 server request routing 由 Main 和 AI-free client 共同拥有；provider compatibility package 不应承担桌面协议实现。
...
遇到“发送无回复”“模型不可用”“custom provider 不生效”时按链路排查：Renderer 是否发出 `codex-chat:start` -> Main 是否通过 shared schema 并进入 runtime -> `HostCodexConnection` 是否完成版本探测和唯一 `initialize` -> 模型目录是否解析到选中模型 -> native driver 是否发送正确的 `thread/start`/`thread/resume`/`turn/start` -> app-server thread config 是否包含期望的 `model_provider`/`model_providers` -> app-server 是否请求 custom provider -> 中性事件是否转换为 `UIMessageChunk` 并经 MessagePort 回到 renderer。
